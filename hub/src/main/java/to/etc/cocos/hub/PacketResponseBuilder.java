...
final public class PacketResponseBuilder {
	...
	public void send() {
		...
		if(null != socketHandler) {
			TxPacket p = new TxPacket(m_envelope.build(), null, m_bodySender, m_onAfter);
			socketHandler.immediateSendPacket(p);
		} else if(null != connection) {
			TxPacket p = new TxPacket(m_envelope.build(), connection, m_bodySender, m_onAfter);
			connection.sendPacket(p);
		} else {
			throw new IllegalStateException("Neither a connection nor a SocketHandler present - cannot send packet");
		}
	}
